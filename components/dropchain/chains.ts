...
export const CHAINS_ARRAY = [
  ...
  { content: 0, color: 'text-green-600' },
];

// Get a random chain, except for the first which is the placeholder and the last which is the merged chain
export const randomChain = (): Chain =>
  CHAINS_ARRAY[Math.floor(Math.random() * (CHAINS_ARRAY.length - 2) + 1)];
